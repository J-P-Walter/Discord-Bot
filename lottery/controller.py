...
class LotteryController:
    ...
    def get_drawing_results(self):
        previous_drawing = LotteryDrawing.objects.filter(dtype=LotteryDrawing.TYPE_SYSTEM).order_by("-created_at")[:2][1]
        all_tickets = LotteryDrawing.objects.filter(created_at__gt=previous_drawing.created_at, dtype=LotteryDrawing.TYPE_USER)
        results = {
            '2': 0,
            '3': 0,
            '4': 0,
